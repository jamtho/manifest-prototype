...
**The SDL vocabulary will evolve.** The current vocabulary covers schemas, semantic types, constraints, derivations, aggregation relationships, physical layout, provenance, and known deficiencies. Modelling a very different domain (e.g. prediction market data) will likely reveal gaps, especially in the constraint combinator algebra and the aggregation function vocabulary. This is expected and welcome — the vocabulary is designed to be extended without breaking existing descriptions.
...
4. Run `sdl validate` — all standard checks (schema, ranges, ordering) work automatically
5. For domain-specific constraints, implement validators and register them via `ValidatorRegistry`

The core vocabulary (`sdl_core.ttl`) should not need to change for new domains. If it does, that's a signal that something belongs in the core that was missed.
